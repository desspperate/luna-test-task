...
class TestGetExisting:
    async def test_returns_200_with_full_payload(self, api_client) -> None:
    ...
    async def test_returned_id_round_trips_as_uuid(self, api_client) -> None:
        created = await api_client.post(
            "/api/v1/payments",
...
class TestNotFound:
    async def test_unknown_id_returns_404(self, api_client) -> None:
        resp = await api_client.get(f"/api/v1/payments/{uuid4()}")
        assert resp.status_code == 404

    async def test_404_uses_payment_not_found_code(self, api_client) -> None:
        resp = await api_client.get(f"/api/v1/payments/{uuid4()}")
        assert resp.json()["code"] == "PAYMENT_NOT_FOUND"


class TestPathValidation:
    async def test_invalid_uuid_returns_422(self, api_client) -> None:
